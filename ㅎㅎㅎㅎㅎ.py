...
def plot_trigonometric_function():
    ...
    # x 값의 범위 설정 (라디안)
    # 그래프를 더 잘 볼 수 있도록 기본 범위를 -2π에서 2π로 설정했습니다.
    ...
    if func_type == 'sin':
        ...
        # 점근선은 bx + c = n*pi + pi/2 일 때 발생합니다.
        
        # 탄젠트 계산
        y_raw = a * np.tan(b * x + c) + d
        
        # 특정 범위 밖의 값을 NaN으로 처리하여 점근선을 시각적으로 표현
        # 너무 큰 값이나 작은 값을 NaN으로 만들어 그래프에서 해당 지점을 그리지 않습니다.
    ...
    # y축 범위는 함수에 따라 유동적으로 조절될 수 있도록 기본값을 제거했습니다.
    # 하지만 특정 범위를 원하면 plt.ylim([-Y_MIN, Y_MAX])와 같이 다시 추가할 수 있습니다.
